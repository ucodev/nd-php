			...
			<?php if (count($view['result_array'])): ?>
				...
				<script type="text/javascript">
					...
					ndphp.ajax.update_data_result = function() {
						jQuery.ajax({
							type: "POST",
							url: "<?=filter_js_str(base_url(), $config['charset'])?>index.php/<?=filter_js_str($view['ctrl'], $config['charset'])?>/result_data_ajax/query/<?=filter_js_str($view['result_query'], $config['charset'])?>/<?=filter_js_str($config['order_by'], $config['charset'])?>/<?php echo($config['order'] == 'asc' ? 'desc' : 'asc');?>/<?=filter_js_str($view['page'], $config['charset'])?>",
							success: function(data) {
								var html = jQuery(data);
								ndphp.nav.back_store('result', jQuery('#result').html());
								jQuery("#result").fnd_animate_hide(600, function() {
