 ...
 <div id="result" class="result">
	...
	<div class="tab-content">
		<div class="tab-pane fade active in" id="listing">
				...
				<?php foreach ($view['grouping_result_array'] as $group => $result_array): ?>
					<ul class="list-group" id="group_<?=filter_html_special($view['grouping_hashes'][$group], $config['charset'])?>"> <!-- Begin of Group container div -->
						<a href="javascript:void(0);" onclick="ndphp.grouping.group_visibility_toggle(event, '<?=filter_html_js_str($view['ctrl'], $config['charset'])?>', '<?=filter_html_special($view['grouping_hashes'][$group], $config['charset'])?>');" class="group_header_link">
							<li class="list-group-item" id="group_header_<?=filter_html_special($view['grouping_hashes'][$group], $config['charset'])?>" class="group_header">
						...
						<div id="group_data_<?=filter_html_special($view['grouping_hashes'][$group], $config['charset'])?>" style="display: none;">
							<div class="table-responsive" style="padding-top: 20px;">
								...
								<?php foreach ($row as $field => $value): ?>
									...
									?>
									<th>
										<a href="<?=filter_html(base_url(), $config['charset'])?>index.php/<?=filter_html($view['ctrl'], $config['charset'])?>/result_group/<?=filter_html($view['grouping_field'], $config['charset'])?>/query/<?=filter_html($view['result_query'], $config['charset'])?>/<?=filter_html($field, $config['charset'])?>/<?=filter_html($config['order'], $config['charset'])?>/<?=filter_html($page, $config['charset'])?>" onclick="ndphp.ajax.load_group_data_ordered_result(event, '<?=filter_html_js_str($view['ctrl'], $config['charset'])?>', '<?=filter_html_js_str($view['grouping_field'], $config['charset'])?>', '<?=filter_html_js_str($view['result_query'], $config['charset'])?>', '<?=filter_html_js_str($field, $config['charset'])?>', '<?=filter_html_js_str($config['order'], $config['charset'])?>', '<?=filter_html_js_str($page, $config['charset'])?>');" title="<?=filter_html(NDPHP_LANG_MOD_OP_LIST_ORDER_BY, $config['charset'])?> <?=filter_html(ucfirst(isset($view['fields'][$field]['viewname']) ? $view['fields'][$field]['viewname'] : $field), $config['charset'])?>">
											...
											<td class="list_op">
												<?php foreach ($view['links']['quick'] as $link):
														...
														if (!security_perm_check($security['perms'], $link[1], $view['ctrl']))
															continue;
												?>
													<a href="<?=filter_html(base_url(), $config['charset'])?>index.php/<?=filter_html($view['ctrl'], $config['charset'])?>/<?=filter_html($link[2], $config['charset'])?>/<?=filter_html($row['id'], $config['charset'])?>" title="<?=filter_html($link[0], $config['charset'])?>" onclick="ndphp.modal.show(this.href, {title: this.title, width: <?=filter_html_js_special($link[4], $config['charset'])?>}); return false;">
			...
			<?php endif; ?>
			<!-- Result Query -->
			<input id="result_query" type="hidden" value="<?=filter_html($view['result_query'], $config['charset']) . '/' . filter_html($config['order_by'], $config['charset']) . '/' . ($config['order'] == 'asc' ? 'desc' : 'asc') . '/' . filter_html($page, $config['charset'])?>" />
		</div>
		<div id="charts">
