...
<div id="create" class="create">
	...
	<form action="<?=filter_html(base_url(), $config['charset'])?>index.php/<?=filter_html($view['ctrl'], $config['charset'])?>/insert" id="createform" name="createform" enctype="multipart/form-data" method="post">
					...
					<table class="fields">
						...
						<?php if ($meta['input_type'] == 'checkbox'): ?>
								<tr id="<?=filter_html_special($field, $config['charset'])?>_row" class="field_<?php echo($i % 2 ? 'even' : 'odd'); ?>">
									...
									<td class="field_value">
										...
										<input alt="<?=filter_html(ucfirst($meta['viewname']), $config['charset'])?> <?=in_array($field, $view['required']) ? '(' . filter_html(NDPHP_LANG_MOD_WORD_REQUIRED, $config['charset']) . ')' : ''?>" name="<?=filter_html($field, $config['charset'])?>" type="checkbox" value="1" <?=$i ? '' : 'autofocus accesskey="' . filter_html(NDPHP_LANG_MOD_OP_ACCESS_KEY_AUTOFOCUS, $config['charset']) . '"'?> <?=isset($view['autocomplete'][$field]) && $view['autocomplete'][$field] == '1' ? 'checked' : ''?> />
									...
									<td class="field_value">
										<select id="<?=filter_html_special($field, $config['charset'])?>" name="<?=filter_html($field, $config['charset'])?>" <?php if ($config['choices'] && in_array($field, $choice_select)) { echo('onchange="selected_' . filter_html_js_special($view['ctrl'], $config['charset']) . '_create_choice_' . filter_html_js_special($field, $config['charset']) . '();"'); }?> <?=$i ? '' : 'autofocus accesskey="' . filter_html(NDPHP_LANG_MOD_OP_ACCESS_KEY_AUTOFOCUS, $config['charset']) . '"'?> >
											<?php foreach ($meta['options'] as $opt_id => $opt_value): ?>
													<option <?=((isset($view['default'][$field]) && $view['default'][$field] == $opt_id) || (isset($view['autocomplete'][$field]) && $view['autocomplete'][$field] == $opt_id)) ? 'selected="selected"' : ''?> value="<?=filter_html($opt_id, $config['charset'])?>">
						...
						<?php elseif ($meta['input_type'] == 'timer'): ?>
								<tr id="<?=filter_html_special($field, $config['charset'])?>_row" class="field_<?php echo($i % 2 ? 'even' : 'odd'); ?>">
									<td class="field_name"><?=filter_html(ucfirst($meta['viewname']), $config['charset'])?>  <?php echo(in_array($field, $view['required']) ? '*' : NULL); ?></td>
									<td class="field_value">
										<input id="<?=filter_html_special($field, $config['charset'])?>" name="<?=filter_html($field, $config['charset'])?>" type="text" alt="<?=filter_html(ucfirst($meta['viewname']), $config['charset'])?> <?=in_array($field, $view['required']) ? '(' . filter_html(NDPHP_LANG_MOD_WORD_REQUIRED, $config['charset']) . ')' : ''?>" value="<?=isset($view['autocomplete'][$field]) ? filter_html($view['autocomplete'][$field], $config['charset']) : '00:00:00'?>" <?php echo(in_array($field, $view['required']) ? 'required="required"' : NULL); ?> <?=$i ? '' : 'autofocus accesskey="' . filter_html(NDPHP_LANG_MOD_OP_ACCESS_KEY_AUTOFOCUS, $config['charset']) . '"'?> />
						...
						<?php elseif ($meta['input_type'] == 'textarea'): ?>
								<tr id="<?=filter_html_special($field, $config['charset'])?>_row" class="field_<?php echo($i % 2 ? 'even' : 'odd'); ?>">
									<td class="field_name"><?=filter_html(ucfirst($meta['viewname']), $config['charset'])?>  <?php echo(in_array($field, $view['required']) ? '*' : NULL); ?></td>
									<td class="field_value">
										<textarea id="<?=filter_html_special($field, $config['charset'])?>" name="<?=filter_html($field, $config['charset'])?>" alt="<?=filter_html(ucfirst($meta['viewname']), $config['charset'])?> <?=in_array($field, $view['required']) ? '(' . filter_html(NDPHP_LANG_MOD_WORD_REQUIRED, $config['charset']) . ')' : ''?>" placeholder="<?=$meta['placeholder'] ? filter_html($meta['placeholder'], $config['charset']) : filter_html($view['default'][$field], $config['charset'])?>" <?php echo(in_array($field, $view['required']) ? 'required="required"' : NULL); ?> <?=$i ? '' : 'autofocus accesskey="' . filter_html(NDPHP_LANG_MOD_OP_ACCESS_KEY_AUTOFOCUS, $config['charset']) . '"'?> ><?=isset($view['autocomplete'][$field]) ? filter_html($view['autocomplete'][$field], $config['charset']) : ''?></textarea>
										<?php if ($meta['help_desc'] != NULL): ?>
											<a href="<?=filter_html($meta['help_url'], $config['charset'])?>" title="<?=filter_html($meta['help_desc'], $config['charset'])?>">
												<img src="<?=filter_html(static_images_url(), $config['charset'])?>/themes/<?=filter_html($config['theme']['name'], $config['charset'])?>/icons/help_small.png" alt="<?=filter_html($meta['help_desc'], $config['charset'])?>" />
											</a>
		...
		<div class="create_ops">
			<button id="op_submit" type="submit" value="<?=filter_html(NDPHP_LANG_MOD_OP_CONTEXT_CREATE, $config['charset'])?>" title="<?=filter_html(NDPHP_LANG_MOD_OP_CONTEXT_CREATE, $config['charset'])?>" class="context_menu_button">
				<?=filter_html(NDPHP_LANG_MOD_OP_CONTEXT_CREATE, $config['charset'])?>
			</button>
			<a href="<?=filter_html(base_url(), $config['charset'])?>index.php/<?=filter_html($view['ctrl'], $config['charset'])?>/list_default" onclick="ndphp.form.cancel_create(event, '<?=filter_html_js_str($view['ctrl'], $config['charset'])?>', <?=isset($config['modalbox']) ? 1 : 0?>);" title="<?=filter_html(NDPHP_LANG_MOD_OP_CONTEXT_CANCEL, $config['charset'])?>" class="context_menu_link">
