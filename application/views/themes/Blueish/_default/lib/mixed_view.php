...
?>
			<script type="text/javascript">
				...
				jQuery.ajax({
					type: "POST",
					url: "<?=filter_js_str(base_url(), $config['charset'])?>index.php/<?=filter_js_str($meta['rel_table'], $config['charset'])?>/view_mixed_rel_count/<?=filter_js_str($view['ctrl'], $config['charset'])?>/<?=filter_js_str($view['id'], $config['charset'])?>",
					success: function(data) {
						mixed_populate_items_<?=filter_js_special($meta['rel_table'], $config['charset'])?>(parseInt(data));
					},
					error: function(xhr, ajaxOptions, thrownError) {
						jQuery("#ajax_error_dialog").html('<?=filter_html_js_str(NDPHP_LANG_MOD_UNABLE_RETRIEVE_RELATED_ITEM, $config['charset'])?> <?=filter_html_js_str($meta['rel_table'], $config['charset'])?>.<br /><br /><span style="font-weight: bold"><?=filter_html_js_str(ucfirst(NDPHP_LANG_MOD_WORD_REASON), 'UTF-8')?>:</span> ' + xhr.responseText);
			...
			<div id="mixed_<?=filter_html_special($meta['rel_table'], $config['charset'])?>_container" style="display: table; margin: 0 auto; padding-top: 34px;">
				<fieldset class="view_mixed_fieldset">
					<legend class="view_mixed_legend">
