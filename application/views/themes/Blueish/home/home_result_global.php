	 	...
	 	<table class="result">
		 	<?php $i = 0; foreach ($view['result_array'] as $row): ?>
		 		<tr class="result_<?=($i % 2) ? 'even' : 'odd'?>">
		 			...
		 			<td class="result">
		 				<?=filter_html(NDPHP_LANG_MOD_SEARCH_GLOBAL_NRESULT_PREFIX, $config['charset'])?> <?=count($row['result'])?> <?=filter_html(NDPHP_LANG_MOD_SEARCH_GLOBAL_NRESULT_SUFFIX, $config['charset'])?>
		 			</td>
		 		</tr>
		 	<?php $i ++; endforeach; ?>
	 	</table>
